...
- 使用者希望的結果：新增fields／hide／separator、pair／tuple單格格式及雙層@segment，並以新語法改寫兩個範例；`+=`應讓來源格內數字移至目的數字位置後提交結果；`target = a + b`應讓兩個可見來源數字同步移向目的格，抵達時消失並提交加總；全域純量跨幀保持同一物件；自動固定與迴圈邊界設定應跟著程式設定檔保存。
