...
- 驗證時的 HEAD 與未提交修改：79073d3e3e05b3f4c97c8218033b8c2dd6e3102e；程式驗證完成時工作目錄乾淨
- 驗證日期：2026-09-19
...
- 修正方式與行為變化：新增 fields／hide／separator 解析、runtime 保存及 heap 同格組合；pair／tuple 以單格格式化；新增雙層 @segment 的局部座標、裁切、重疊、色彩及具名轉場；with split(cursor[,after])依遞迴路徑保留尚待處理的另一側；保留舊單層 @segment。兩個 Segment Tree 範例改用 render heap 與新指令。
...
- 實際結果與 exit code：PID 60868；來源 hash 相符；analyze 回傳split cursor=now、phase=before且style color為空；renderer trace-195、tween trace-213、model trace-33、directive assist directive-15。
