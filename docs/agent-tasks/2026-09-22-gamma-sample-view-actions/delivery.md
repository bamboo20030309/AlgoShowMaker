...
- 程式修正 commit：77815e58d81b9fc8665c29b939b7959ab668e45f、fc3e15a0d002ad302809b075b99224ba8101c067
- 驗證時的 HEAD 與未提交修改：HEAD `fc3e15a0d002ad302809b075b99224ba8101c067`；只有本交付文件更新尚未提交。
- 驗證日期：2026-09-22

## 根因與修改
- 已確認根因與證據：範例觀賞模式以 `?sample=` 識別，但左上角 `.chrome-home-link` 的 `href`、標題與無障礙名稱固定指向一般首頁。
- 修正方式與行為變化：保留既有範例匯出與分享；載入 `?sample=` 時將首頁按鈕改為 `/?examples=1`，標示「返回範例投影片」。一般 deck 與 `?share=` 仍返回 `/`。
- 修改檔案及用途：`public/slides.html` 為首頁按鈕加入穩定 ID 並更新資產版本；`public/slides.js` 依 `sampleId` 設定目的地與文案；`tests/sample-view-actions.browser.test.js` 實際點擊並確認範例列表模式；`tests/entrypoints.test.js` 核對版本。
...
| 範例返回範例列表，其他共享投影片仍回首頁 | 專項測試檢查連結文案並實際點擊 | 範例導向 `/?examples=1` 且 body 進入 `examples-view`；共享頁仍為 `/` | 通過 |
...
- 完整指令或操作步驟：`node --test tests/sample-view-actions.browser.test.js`；`node --test tests/entrypoints.test.js`
- 預期結果：兩個專項測試通過。
- 實際結果與 exit code：兩個測試各 1/1 通過，0 failed，exit code 0；範例返回導覽包含實際頁面跳轉。
...
- 已知問題或風險：無。
- 相依與衝突注意：若主代理整合時 `slides.html` 的資產版本已前進，保留新的快取值並同步入口測試；目前為 `slides.js?v=inline-scripts-216`。
- 主代理需補驗證的情境：整合後從真實範例列表開啟任一範例，再點左上角首頁圖示確認回到範例列表。

## 主代理核實與整合（由主代理填寫）
- 狀態：先前匯出／分享已核實並合併至 `intergration`；本次返回範例列表的後續修改尚待主代理核實。
