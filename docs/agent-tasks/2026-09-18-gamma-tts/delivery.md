...
- 程式修正 commit：ae6b9f6c94c65f996289ad78cd4d101b43b0c09c
- 驗證版本：上述程式內容；驗證時未提交，後續 commit 未改程式。文件另提交。
- 日期：2026-09-18

## 根因與修改
TTS 按鈕與面板都在觀賞模式隱藏的側欄內。新增右上 TTS 小按鈕，沿用同一面板，觀賞時搬到 body 顯示為浮動面板，回 Edit 搬回原側欄；保留既有播放／暫停／續播／停止與語速音量。隱藏觀賞面板的旁白新增與編輯列表，共享唯讀語速音量不回存 deck。
...
- 使用說明：觀賞页右上 TTS 展開；播放按鈕可暫停／續播，停止按鈕結束，滑桿調整語速與音量。關閉面板會停止播放（沿用既有行為）。
- 與 task.md 差異：無。
...
- 指令（自己 worktree 的 algo-vis-backend）：node --check public/slides.js；git diff --check；node --test tests/entrypoints.test.js；node test-results/viewer-tts-check.cjs。
...
- 證據：本 worktree algo-vis-backend/test-results/viewer-tts-check.cjs、viewer-tts-mobile.png；本機、不提交，移除 worktree 後不保證留存。
