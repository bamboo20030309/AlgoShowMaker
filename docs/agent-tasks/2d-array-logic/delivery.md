...
- 四步公式的來源項改為展開後的索引名稱，例如 `pre[2][2] = pre[1][2] + pre[2][1] - pre[1][1] + num[2][2] = 16`；不再用單獨數字取代來源格名稱，四色底色仍對應各來源項。

## 2026-09-25 鏡頭與文字背景修正

- 依使用者貼上的版本更新範例：預設全景 zoom 改為 `1`、第 0 列／欄完整標灰、聚焦倍率改為 `1.7`，並加入範例的 `@asm-view` 設定。
