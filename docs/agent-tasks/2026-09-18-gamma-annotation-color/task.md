...
## 2026-09-18 後續需求：白色預設
- 使用者要求註標箭頭預設改為白色；新建及缺少annotationColor的物件使用#ffffff，保留已選色物件。
- 本次共同基準：800a357bbf5d30001fa8653215bc2f8426862ab1。
- 修改範圍：slides.js、slide-structures.js的annotationColor預設，slides.html初始色及快取，相關小測試。
- 驗收：未指定色的箭頭為白色；仍可改色並保存重開。
- 驗證：V1，沿用原隔離structure與入口測試，不啟動演算法集。
- 使用說明：不同樣式欄填各自索引可分別套用；同類樣式只有一個共用顏色，不提供同類每格獨立色。
