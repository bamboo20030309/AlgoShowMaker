...
- 程式修正 commit：`6f7acdb49859b88d27528b9f06a432514976d53e`、`29aa32d92cf5768fdbc57d58035b513ee09f6ff5`、`bb428c4e6cf7e32bbfb07544cb473941975b038a`、`286ca4bbe7703304bf8d5fb0eadae177f8f3dbd7`、`28d510d13dadbf574bf43292c0dcfee01a31d8f7`、`0ea000199e178a11cb735e80deab6848e1b716e0`、`71f46f650e7499b14a422b1f86bf8080f79566da`、`c16a32d4a408e228b423ff43923166121f0a842b`
- 驗證時的 HEAD 與未提交修改：`c16a32d4a408e228b423ff43923166121f0a842b`；程式工作樹無未提交修改，僅本交付文件後續更新
- 驗證日期：2026-09-21

## 根因與修改
- 已確認根因與證據：上一幀仍把目的幀與來源幀送入 tween，因此會建構反向幾何；連續下一步原先在前端 promise queue 等候前一段動畫完成，無法把第二次輸入即時交給播放器；移除 queue 後若「穩定目前幀」與「播放下一幀」仍以微任務串接，第三次快轉可先到較後幀，再被舊微任務拉回；marker group 以名稱排序；assignment 讓來源與目的群組在整段 motion 同時重排；reference parameter 使用不同 variable id，導致同一實體被判定為新 visual；樣式補間也只以新 variable id 尋找上一幀，在 heap sort 第 60→61 幀從 `heapify:arr` 返回 `heap_sort:arr` 時找不到相同容器的灰色 focus，因而誤從白色補間。使用者後續提供的第 34→35 幀路徑進一步確認：目的幀 renderer 已依 runtime identity 累積 index 10 的 fixed mark，但 tween 重新呼叫一般 rules evaluate 時沒有再次加入累積 fixed state，`updatePresentedHints` 因此把原勾選設為 `display:none` 且沒有產生替代勾選。第 49→50 幀則是同幀 swap 與新 fixed state 的交互問題：tween 已把 renderer 原先建立的 authored mark 設為延後顯示，但 `updatePresentedHints` 把它隱藏後另建 live mark；新節點錯過延後處理並跟隨 swap 暫時 presentation cell，因而飛到右下方。背景分頁恢復後會把隱藏時間計入動畫。後續 heap 案例另確認遞迴 `int i` 雖視為角色延續，仍套用宣告 marker 的 180ms 位移排程，且目的格讓位仍使用接近一個標籤寬度的舊 D04 規則。
- 修正方式與行為變化：新增穩定幀導覽入口；上一幀、時間線、快轉與隱藏期間導航皆取消動畫後直接重建；動畫進行中再次按下一步時，前端立即把輸入交給播放器，播放器同步取消舊 tween、穩定重建目前目的幀，再從該幀啟動下一段動畫，避免舊 promise 覆蓋新導覽；樣式補間改用容器 runtime identity 對應 reference alias 的上一幀 cell，使相同 focus 色彩保持不動，且自動固定標記跨 alias 邊界持續可見；heap swap 同幀新增 fixed mark 時，renderer 將 authored hint 節點直接納入 presented hints，保留 tween 已設定的延後 opacity，交換結束後才顯示；500ms 內第 3 次下一步切為逐次穩定幀快轉；按住按鈕或方向鍵使用語速間隔 repeat；marker 依宣告順序排列，來源立即回填，目的格從移動開始同步讓位；宣告事件延續 marker 且有實際位移時採 520ms，無位移維持 180ms；跨格箭頭由來源相對方向依位移進度連續插值至目的相對方向，抵達後才提交邏輯歸屬；局部 reference alias 沿用 visual 並只改名，全域 alias 同幀保留兩個 marker；動畫與 TTS 隨分頁可見性一起暫停。
...
| heap sort 第 34→35、60→61 幀 alias 樣式與自動固定 | 指定 heap sort 實際 trace、`reference-alias-style.browser.test.js` | 時間線穩定跳到第 34 幀後按下一步，index 10 的 fixed mark 在立即、轉場中與穩定第 35 幀皆為可顯示且有非零 bounds；未改變的灰色 focus 在 0/40/120ms 維持 `rgb(204, 204, 204)` | 通過 |
| heap sort 第 49→50 幀 swap fixed mark | `reference-alias-style.browser.test.js` 的 heap swap 案例 | swap 暫時 presentation cell 存在時 index 7 mark 為隱藏；轉場完成後 mark 顯示且中心點位於實際 index 7 格內 | 通過 |
...
- 執行的測試檔／名稱篩選：`tests/unresolved-markers.test.js` 全檔；`tests/trace-player-navigation.test.js`；`tests/playback-navigation.browser.test.js`；`tests/reference-alias-style.browser.test.js`；`tests/event-defaults.test.js`；`tests/entrypoints.test.js`。
- 驗證環境與隔離服務：beta worktree、3102、headless Edge；未操作使用者分頁；3102 已重啟為 PID 50920，並載入 `trace-frame-tween.js?v=trace-215`、`trace-renderer.js?v=trace-188`、`trace-player.js?v=trace-25`、`front.js?v=random-id-34`。
- 驗證版本、完整指令、結果與證據：marker 程式 commit `286ca4bbe7703304bf8d5fb0eadae177f8f3dbd7`；播放中斷程式 commit `28d510d13dadbf574bf43292c0dcfee01a31d8f7`；alias paint 程式 commit `0ea000199e178a11cb735e80deab6848e1b716e0`；alias fixed mark 程式 commit `71f46f650e7499b14a422b1f86bf8080f79566da`；heap swap fixed mark 程式 commit `c16a32d4a408e228b423ff43923166121f0a842b`。分別執行 `node --check public/trace-frame-tween.js`、`node --check public/trace-renderer.js`、`node --check public/front.js`、`node --check public/trace-player.js`、`node --check tests/reference-alias-style.browser.test.js`，另執行 `node --test tests/unresolved-markers.test.js`（60/60）、`node --test tests/trace-player-navigation.test.js tests/entrypoints.test.js`（4/4）、`node --test tests/playback-navigation.browser.test.js`（1/1）、`node --test --test-concurrency=1 tests/reference-alias-style.browser.test.js tests/event-defaults.test.js tests/entrypoints.test.js`（8/8）、重啟 3102 後 `node --test --test-concurrency=1 tests/reference-alias-style.browser.test.js`（2/2）及 `git diff --check`，均通過。
- 未執行的驗證及原因：依驗證分級不執行完整 regression、全部 tests 或大規模演算法動畫。
- 需要主代理做的 V3 驗證：整合後實際播放 heap 第 16→17 幀，觀察 `i` 的 520ms 位移與目的格從起步同步讓位。

### reference alias 固定標記
- 目的與對應條件：重現使用者指定的「切到第 34 幀，再按一次下一步」，核實 reference parameter 返回呼叫端時累積 fixed mark 不會被隱藏。
- 執行目錄與必要環境設定：beta `3102`、headless Edge。
- 測試資料／fixture：`tests/fixtures/reference-alias-focus.cpp`；另以使用者提供的 heap 程式及輸入 `10`、`5 7 2 1 9 4 11 15 8 6` 做單一路徑重現。
- 完整指令或操作步驟：`$env:ASM_TEST_BASE_URL='http://127.0.0.1:3102'; node --test tests/reference-alias-style.browser.test.js`；實際 heap 路徑以時間線切換至顯示第 34 幀，再對 `#nextBtn` 發出一次按下／放開。
- 預期結果：固定勾選在轉場開始、途中及完成後皆 `display != none`、opacity 1 且 bounds 非零；focus 不閃白。
- 實際結果與 exit code（適用時）：browser test 1/1 通過，exit code 0；heap 第 34→35 幀立即、轉場中與穩定狀態均存在可見 index 10 勾選。
- 證據位置：`algo-vis-backend/tests/reference-alias-style.browser.test.js`；臨時截圖與診斷 log 已依規範刪除。
