...
- [x] 同一容器從 reference parameter 返回呼叫端時，未改變的 focus 色彩不得重新由白色補間；既有自動固定標記在整段轉場保持可見。
- [x] heap swap 與 fixed 事件同幀發生時，新 mark 在交換中的暫時格子位置保持隱藏，交換完成後才顯示於實際目標格。
...
- 2026-09-21：使用者回報 heap sort 第 60→61 幀 focus 顏色閃爍且自動固定消失；調查範圍加入 reference alias 跨函式返回時的樣式與固定標記連續性。
- 2026-09-21：使用者提供可重現路徑「時間線切到第 34 幀後按一次下一步」；確認第 35 幀由 `heapify:arr` 返回 `heap_sort:arr` 時，tween 的即時樣式重算遺漏累積 fixed state，使 renderer 已畫出的勾選被 `display:none` 隱藏，納入同一 alias 驗收條件修正。
- 2026-09-21：使用者回報同一 heap sort 範例第 49→50 幀 swap 時 mark 被拉到陣列右下方；確認新 fixed mark 未沿用已套用延遲顯示的 authored 節點，因而在 swap 暫時 presentation cell 上提前出現，納入 fixed mark 時序驗收。
