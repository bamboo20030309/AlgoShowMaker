...
| 上一步的 heap resize 反向 | 隔離瀏覽器播放跨層 push 與 pop 的反向，逐 32ms 擷取 outerframe、格子、sequence ghost 與 opacity | 反向 push 的 ghost 從 opacity 1 外移 48px 並淡至 0；反向 pop 的補回格從 opacity 0 移入並淡至 1；outerframe 與格子同時 resize | 通過 |
...
- 實際結果與 exit code（適用時）：反向 push 的移除格保持 opacity 1 到 sequence 開始，再外移至約 48px 並淡至 0；反向 pop 的補回格保持 opacity 0，到 sequence 開始後移入並淡至 1；兩者都與 outerframe resize 同步。38→37 與直接開啟 37 均只剩 1 個相同 highlight；腳本 exit code 0。最新 `outerframe-tween` 5 案與 sequence integration 2 案通過。
