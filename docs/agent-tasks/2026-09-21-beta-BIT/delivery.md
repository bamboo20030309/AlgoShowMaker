...
- 修正方式與行為變化：新增 C++ 優先序的整數位元運算解析與求值，讓安全的位元索引可保存實際索引；新增 `render binary indexed tree` 全名與連字號／底線別名，同時保留 bit／fenwick；將範例改成 0-based num、1-based BIT 的 build、sum 與 range sum，只用新指令顯示兩個資料物件；採 int、簡短變數與接近舊版的程式結構。
