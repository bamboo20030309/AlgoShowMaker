...
- 程式修正 commit：`5b5ad128c97cc6651c8229cd41a63ddfd52de704`、`0fd7da94fe675efc311327e6cf83a7d34480a924`、`7e0ac3e57a7bd0faa07a6fd44a7fe9e4a99decde`、`80c3d15ae893f59ff7e7f94efda9459100e4d1e8`、`399071d5125999b932aacf5a30ce572cb18b9e83`、`93b5a32d2fe6aa732579b4c74600802586d985b1`、`1d83fa39e8d5ad4028a0c83a3f50188abb658a4e`、`de9ebaca102e2618bd013a2dc1b2ef3fb5749337`、`a69a43f837508d64b7c9c1a79b3789d83c8a0a87`、`33a56ff3c2ec4d907f21ddfb406dbf2ccd9ab8b4`、`210f1d547fca85f9665a30216bae28eb13e2e917`
- 驗證時的 HEAD 與未提交修改：`210f1d547fca85f9665a30216bae28eb13e2e917`；只有本任務 task／delivery 文件待提交
- 驗證日期：2026-09-21

## 根因與修改
- 已確認根因與證據：指令 tokenizer、parser 與瀏覽器 runtime evaluator 原先只支援算術、比較及邏輯運算，無法處理 `& | ^ ~ << >>`；舊範例依賴 `AV.hpp` 與手寫繪圖程式；marker position 分類只接受 `write + update`，漏掉 `write + compound`；終止 build 的 `i: 8→16` 與下一次 `build(2)` 同幀時，穩定狀態沒有 BIT[16] placement，事件動畫因而誤用線性陣列外插。
...
| 超出可見範圍的 BIT marker | BIT 虛擬格幾何單元案例與第 9→10 幀瀏覽器 transition | BIT[16] 依 lowbit 層級位於 BIT[8] 上一層；整段 marker 中心不超出 BIT 水平邊界 | 通過 |
...
- 實際結果與 exit code（適用時）：錨點整體相關案例 16 passed、0 failed、0 skipped；BIT 虛擬格修正後的 marker、BIT 與入口專項 69 passed、0 failed、0 skipped。3102 回應 HTTP 200，載入 `trace-frame-tween.js?v=trace-223` 與 `trace-renderer.js?v=trace-203`。
...
- 驗證環境與隔離服務：先用 3197 隔離服務驗證並停止；最終從 beta BIT worktree 重啟 3102（PID 51556）後重跑。未操作使用者分頁或資料。
- 驗證版本、完整指令、結果與證據：最新程式 commit `210f1d547fca85f9665a30216bae28eb13e2e917`；`unresolved-markers`、BIT parser／browser 與入口專項在重啟後共 69/69 通過。
