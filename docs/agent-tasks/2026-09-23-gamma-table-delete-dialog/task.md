...
- 本次範圍與必要限制：前端功能；只跑相關局部驗證，不跑大規模 regression。Table 不提供 Structure 的 highlight、focus、point、mark、background 或註標 style；只保留表頭、內容、框線、文字等表格配色。表格第一版不含合併儲存格、公式、試算表貼上解析、每格富文字或專用演算法動畫。
...
- [ ] 點擊 Table 格子不顯示 Structure style 工具列，且舊表格資料中的 highlight 等 Structure style 不再渲染並於下次保存時移除。
...
- 2026-09-23：使用者確認 Table 不應包含 highlight 等 Structure style；移除 Table 的 style 工具列、渲染與持久化欄位，保留表格本身配色。
