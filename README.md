...
- `render segment_tree with range(1,n)` 會以 `tree[1]` 為根，依每個節點代表的實際區間決定格子寬度，並沿用標準遞迴深度排列；非二次方長度不補假葉節點，也不把較早結束的葉節點強制推到底層。最小格為 40×40px，下方 index 格高 12px；value 一般使用 16px，tree index 置中，interval 靠右且比 index 小 2px，葉節點 `[x,x]` 簡化為 `[x]`。
