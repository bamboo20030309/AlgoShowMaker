...
- 以註解式指令描述 `@frame`、`@object`、`@style`、`@arrow`、`@camera`、`@keep`、`@layout` 等視覺規則，不需改寫原本的演算法流程。
- Trace Studio 可調整逐幀物件、事件、鏡頭、轉場與播放設定，並將設定保存回 `@asm-view`。
- 演算法編輯器、投影片編輯器與投影片播放共用相同 trace、renderer 與 tween 行為。
- 支援陣列、矩陣、Heap、Binary Indexed Tree、標準 Segment Tree、Stack、Queue、Disk 與遞迴排版等資料結構。
- 標準 Segment Tree 的長度代表根節點 `index(1)` 的資料區間，系統會自動建立所需儲存格並保留舊投影片相容性。
- 投影片可加入文字、LaTeX、程式碼、表格與結構元件，支援排序、多選刪除、復原／重做、TTS、分享與 `.asmdeck` 匯入匯出。
- 內建排序、Heap、Binary Indexed Tree、Segment Tree 與線篩等範例投影片，可從首頁的「範例投影片」入口瀏覽。
- 提供桌面與手機介面；完整語法與案例請參考[演算法視覺化指令使用手冊](ALGORITHM_VISUALIZATION_DIRECTIVE_MANUAL.md)。
