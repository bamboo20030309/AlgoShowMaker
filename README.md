...
- `with`：傳入 `range(...)`、`columns(...)`、`labels(...)` 等 renderer 選項；標準線段樹另使用 `domain(...)`、`root(...)`、`unit(...)`。
...
- `render segment_tree with domain(1,n)` 會依每個節點代表的實際區間決定格子寬度，並沿用標準遞迴深度排列；非二次方長度不補假葉節點，也不把較早結束的葉節點強制推到底層。`root(1)`可改根索引，`unit(48)`可調整每個最小區間的寬度。
