...
- `Segment_Tree_standard.cpp` 示範標準遞迴 lazy segment tree；輸入先給 `n m` 與 n 個初值，再以 `1 L R value` 表示區間加值、`2 L R value` 表示區間設值、`3 L R` 表示區間總和查詢。範例用 `fields(tree,lazy,sets)` 將三個欄位放進同一格，並以 `hide(lazy=0,sets=LM)` 隱藏預設標記。更新回朔時會顯示左右子節點相加寫回父節點；查詢跨中點時則顯示左右回傳值相加的回朔幀。兩種回朔幀都以 `split(now,after)` 同步收回已完成節點的 segment。
