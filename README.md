...
登入後「我的投影片」可新增單層資料夾。拖曳縮圖或縮圖下方的拖曳把手至另一張縮圖前方可排序，拖到資料夾標題列或空白區可放入該資料夾；也可用資料夾選單與前後按鈕整理。資料夾和順序儲存於帳號，未分類會收納新增的檔案；移除資料夾會將檔案移回未分類，不會刪除投影片。範例頁面與個人工作區的資料夾皆以橫線分隔，點標題可展開或收合，點分類導覽可移至該區塊。
