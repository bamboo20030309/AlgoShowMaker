    ...
    void prime_table() {
        ...
        av.start_frame_draw();
        av.colored_text({{"首先因為"},{"1不是質數","rgba(241, 255, 47, 0.44)"},{"所以先刪掉"}},Pos(0,0));
        ...
        for(int i=2;i<=n;i++){
            ...
            if(i==21)av.skip();
            av.start_frame_draw();
            if(isprime[i])av.colored_text( {{"因為 "},{"{isprime}["+to_string(i)+"]={true:是質數}","rgba(47, 255, 82, 0.44)"},{" 所以將其放入 prime" }} ,Pos(0,0));
            else          av.colored_text( {{"因為 "},{"{isprime}["+to_string(i)+"]={false:不是質數}","rgba(234, 64, 64, 0.44)"},{" 所以不是質數"}},Pos(0,0));
            string _key_text = AV::array_to_string(_key_frame_text);
            if(isprime[i])av.key_colored_text( {{"因為 "},{"{isprime}["+to_string(i)+"]={true:是質數}","rgba(47, 255, 82, 0.44)"},{" {所以}將其放入 prime\n並且一口氣殺掉"+to_string(i)+"{×:乘}"+_key_text+" {這幾個}" }}   ,Pos(0,0));
        ...
        //    av.key_arrow(Pos("isprime",i-1), Pos("prime",k),  {{"color","rgba(13, 102, 13, 0.8)"}, {"label","×"}, {"width","3"}});
            int _tmp_k=0;
            for(auto&v:prime){
                if(i*v>n)break;
    ...
    int main(){
        ...
        av.start_draw();
        av.start_frame_draw();
        av.text("這是線篩{的演算法視覺化範例}",Pos(0,0));
        av.frame_draw(  "prime" , Pos(0,560),     prime, {},                        {0}  , "normal", _draw_itemsPerRow, 0);
