...
int query(int l,int r,int L,int R,int now=1){ //如果要改成min的話 Add要改成LM
    if(L<=l && r<=R){ //所有尾端的節點 必須在這處理並停止
        ...
        av.start_frame_draw();
        av.frame_draw("tree" , Pos(0,0),    tree, {{{"highlight"},{now}}, {{"focus"},_draw_focus}, {{"point"},{now}}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        av.key_frame_draw("tree" , Pos(0,0),       tree, {{{"highlight"},{now}}, {{"focus"},_draw_focus}, {{"point"},{now}}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        av.text("找到符合大小的區段就回傳 "+to_string(tree[now]),Pos("tree","top",0,-60));   
...
int main(){
    ...
    for(int i=0;i<m;i++){
        ...
        av.start_frame_draw();
        av.frame_draw("tree" , Pos(0,0),    tree, {{{"focus"},_draw_focus}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        av.key_frame_draw("tree" , Pos(0,0),       tree, {{{"focus"},_draw_focus} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        ...
        av.start_frame_draw();
        av.frame_draw("tree" , Pos(0,0),    tree, {{{"focus"},_draw_focus}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        av.key_frame_draw("tree" , Pos(0,0),       tree, {{{"focus"},_draw_focus}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, _draw_segment[0], _draw_segment[1], _draw_segment[2]);
        av.colored_text({{{"完成一次 "}},{{"query 的操作\n"},"rgba(18, 221, 35, 0.44)"},{{"區間搜尋結果 "+to_string(x)+"~"+to_string(y)+" 的數值為 "+to_string(ans)},"rgba(18, 221, 35, 0.44)"}},Pos("tree","top",0,-80));
        av.camera(Pos("tree", "top", 0, 120), 1.8);
        av.end_frame_draw();
        //}   
    }
    //draw{
    av.start_frame_draw();
    av.frame_draw("tree" , Pos(0,0),       tree, {{{"focus"},_draw_focus}, {{"seg_bg",_draw_seg_color},{}} }, {1,n}, "segment_tree", 20, 1, 0, lazy, sets, vector<int>(), vector<int>(), vector<int>());
    av.camera(Pos("tree", "top", 0, 120), 1.8);
    av.end_frame_draw();
    av.end_draw();
    //}
