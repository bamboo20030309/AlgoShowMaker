...
void heap_push (int n) {
    ...
    while(now > 1){
        if(heap[now>>1] >= heap[now]) {
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"上層比下層大 "},{"不用交換","rgba(255, 82, 82, 0.44)"}}, Pos(0,-60));
            ...
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"上層",_draw_up_color},{" 比 "},{"下層",_draw_down_color},{" 小的時候"}}, Pos(0,-60));
            ...
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"上層",_draw_down_color},{" 和 "},{"下層",_draw_up_color},{" 就上下交換"}}, Pos(0,-60));
...
int heap_top(){
    //draw{
    av.start_frame_draw();
    av.colored_text( {{"查詢 ","rgba(47, 255, 82, 0.44)"},{" heap中最大元素: "},{to_string(heap[1]),"rgba(47, 255, 82, 0.44)"} }, Pos(0,-60));
...
void heap_pop() {
    ...
    //draw{
    av.start_frame_draw();
    av.colored_text( {{"刪除 ","rgba(255, 82, 82, 0.44)"},{" heap中最大元素: "},{to_string(heap[1]),"rgba(255, 82, 82, 0.44)"} }, Pos(0,-60));
    ...
    //draw{
    av.start_frame_draw();
    av.colored_text( {{"先把 "},{"最前面",_draw_up_color},{" 和 "},{"最後面",_draw_down_color},{" 交換"}}, Pos(0,-60));
    ...
    //draw{
    av.start_frame_draw();
    av.colored_text( {{"{先把} "},{"最前面",_draw_down_color},{" 和 "},{"最後面",_draw_up_color},{" 交換"}}, Pos(0,-60));
    ...
    //draw{
    av.start_frame_draw();
    av.colored_text( {{"{接著}刪除掉最後面的格子"}}, Pos(0,-60));
    ...
    while (now*2+1 <= Size) {
        //draw{
        av.start_frame_draw();
        av.colored_text( {{"看看 "},{"下層的兩個格子",_draw_down_color},{"有沒有比 "},{"上層",_draw_up_color},{" 大"}}, Pos(0,-60));
        ...
        if (heap[now] < Max) {
            if(heap[now<<1] > heap[(now<<1)+1]) {
                //draw{
                av.start_frame_draw();
                av.colored_text( {{"{有的話} "},{"大的那個",_draw_down_color},{" 跟 "},{"上層",_draw_up_color},{" 交換"}}, Pos(0,-60));
                ...
                //draw{
                av.start_frame_draw();
                av.colored_text( {{"{有的話} "},{"{大的那個}",_draw_up_color},{" {跟} "},{"{上層}",_draw_down_color},{" {交換}"}}, Pos(0,-60));
            ...
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"{沒有的話}就不用交換"}}, Pos(0,-60));
    ...
    if (Size == 2) {
        //draw{
        av.start_frame_draw();
        av.colored_text( {{"看看 "},{"下層",_draw_down_color},{" 有沒有比 "},{"上層",_draw_up_color},{" 大"}}, Pos(0,-60));
        av.frame_draw("num" , Pos(0,0),   heap, { {{"highlight"},{now<<1,(now<<1)+1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_up_color},{now}}, {{"background",_draw_down_color},{now<<1,(now<<1)+1}} }, {0}, "normal", 0, 1);
        av.frame_draw("heap", Pos(0,150), heap, { {{"highlight"},{now<<1,(now<<1)+1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_up_color},{now}}, {{"background",_draw_down_color},{now<<1,(now<<1)+1}} }, {0}, "heap",  10, 1);
        av.end_frame_draw();
        //}
        if(heap[now] < heap[now<<1]) {
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"{因為只剩一個就直接} "},{"下層",_draw_up_color},{" 跟 "},{"上層",_draw_down_color},{" 交換"}}, Pos(0,-60));
            av.frame_draw("num" , Pos(0,0),   heap, { {{"highlight"},{now,now<<1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_up_color},{now}}, {{"background",_draw_down_color},{now<<1}} }, {0}, "normal", 0, 1);
            av.frame_draw("heap", Pos(0,150), heap, { {{"highlight"},{now,now<<1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_up_color},{now}}, {{"background",_draw_down_color},{now<<1}} }, {0}, "heap",  10, 1);
            av.end_frame_draw();
            //}
            ...
            //draw{
            av.start_frame_draw();
            av.colored_text( {{"{因為只剩一個就直接} "},{"{下層}",_draw_down_color},{" {跟} "},{"{上層}",_draw_up_color},{" {交換}"}}, Pos(0,-60));
            av.frame_draw("num" , Pos(0,0),   heap, { {{"highlight"},{now,now<<1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_down_color},{now}}, {{"background",_draw_up_color},{now<<1}} }, {0}, "normal", 0, 1);
            av.frame_draw("heap", Pos(0,150), heap, { {{"highlight"},{now,now<<1}}, {{"focus"},{now,now<<1,(now<<1)+1}}, {{"background",_draw_down_color},{now}}, {{"background",_draw_up_color},{now<<1}} }, {0}, "heap",  10, 1);
            av.end_frame_draw();
            //}
...
int main() {
    ...
    av.start_draw();
    av.start_frame_draw();
    av.text("這是堆積(heap){的演算法範例}", Pos(0,-60));
