...
void draw_all_pegs() {
  ...
  draw_single_peg("B", base_x + 250);
  draw_single_peg("C", base_x + 500);
}
...
void hanoi(int n, string from, string to, string aux) {
  ...
  tree.nodes.insert(
      {tree.curr_d, tree.curr_o}); // 先行註冊節點，確保箭頭錨點抓得到座標
  ...
  av.text("已完成上面 " + to_string(n - 1) + " 個盤子的移花步驟，返回主層級。",
          Pos("tree_0_0", "top", 0, -120));
  av.auto_camera();
  av.end_frame_draw();
  tree.pop();
  ...
  av.text("{接木}：\n底盤已歸位，現在要把剛才遞迴移走的 " + to_string(n - 1) +
              " 個盤子，\n從暫存的 " + aux + " 搬回目標 " + to + "...",
          Pos("tree_0_0", "top", 0, -120));
  av.auto_camera();
  av.end_frame_draw();
  ...
  av.text("已將 " + aux + " 暫存的盤子接木完成，返回主層級。",
          Pos("tree_0_0", "top", 0, -120));
  av.auto_camera();
  av.end_frame_draw();
  tree.pop();
...
int main() {
  ...
  av.auto_camera();
  av.text("所有步驟執行完畢，河內塔搬運成功！", Pos("tree_0_0", "top", 0, -90));
  av.end_frame_draw();
  av.end_draw();
  //}
