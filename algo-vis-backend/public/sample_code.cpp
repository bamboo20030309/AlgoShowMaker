...
int main() {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if(i==3 || i==7 || i==13) {
            ...
            av.key_frame_draw("heap", Pos("num","bottom-left",0,100), num, {{{"mark"},AV::AtoB(0,i)}, {{"highlight"},{i-1}}, {{"point"},{i-1}}, {{"focus"},{i-1}}, {{"background"},{i-1}}}, {0},  "heap", 10, 1);
            av.key_frame_draw("BIT", Pos("heap","bottom-left",0,100), num, {{{"mark"},AV::AtoB(0,i)}, {{"highlight"},{i-1}}, {{"point"},{i-1}}, {{"focus"},{i-1}}, {{"background"},{i-1}}}, {0},  "BIT", 10, 1);
        }
        av.end_frame_draw();
    }
    av.end_draw();
