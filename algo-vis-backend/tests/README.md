...
持久化物件新增功能時必須驗證舊資料：新建物件、刪除新欄位的舊 fixture、既有自訂值／明確關閉設定，以及儲存或匯出後重開。已登錄案例可用 `npm run test:legacy-objects` 執行；新增功能仍須先加入對應 fixture 與斷言，不能用既有案例代替本次功能驗證。
