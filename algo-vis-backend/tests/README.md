...
Heap複合格與格內區段專項為`heap-composite-segments.test.js`／`heap-composite-segments.browser.test.js`及`fixtures/heap-composite-segments.cpp`：fields來源、hide、separator、pair／tuple、根與子節點局部座標、裁切／重疊、既有單層segment與實際SVG。`segment-tree-samples.test.js`以兩份sample input核對新版範例不含AV.hpp舊繪圖程式，並確認輸出與特殊線段樹邏輯不變。三檔需設定隔離`ASM_TEST_BASE_URL`。
